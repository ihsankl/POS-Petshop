...
const MenuHeader = props => {
    ...
    const onLogout = async () => {
        ...
        await props.dispatch(getUser({ data: [], isSigned: false }))
    ...
    return (
        <animated.div className={`z-40 fixed right-0 top-0 left-0 overflow-hidden justify-center flex`}>
            <div className={`flex text-center w-3/4 py-4 bg-gray-200 `}>
                ...
                <Link to="/kasir" onClick={() => handleMenu('kasir')} className={`font-bold focus:outline-none flex-1 ${Menu === 'kasir' || Menu === 'Home' ? 'text-purple-500' : 'text-purple-400'}`}>Kasir</Link>
