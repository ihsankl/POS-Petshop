...
const Kasir = (props) => {
    ...
    const pushToInvoice = async (data) => {
        ...
        const x = _.findWhere(list, { nama_barang: data.nama_barang })
        if (!x) { // if data is not exist, add new item
            if (data.sisa_stok > 0) {
                const toProcess = {
                    ...data,
                    qty: 1
        ...
        <div style={{
            ...
            <div className="flex w-3/5 flex-col">
                ...
                <div className="flex flex-wrap mt-8">
                    {props.dataBarang.barang &&
                        props.dataBarang.barang.map((data, index) => (
                            <button key={data.id} onClick={() => pushToInvoice(data)} className="mr-4 mb-4 rounded-md border-2 border-purple-500 w-44 bg-white flex flex-col p-2">
